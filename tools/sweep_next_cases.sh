#!/usr/bin/env bash
set -euo pipefail

if [[ $# -lt 2 || $# -gt 4 ]]; then
  echo "usage: $0 WEIGHT ORBIT_PATH [SECONDS] [JOBS]" >&2
  exit 2
fi

weight="$1"
path="$2"
seconds="${3:-30}"
jobs="${4:-8}"

root="$(cd "$(dirname "$0")/.." && pwd)"
generator="$root/build/generate_cnf"
solver="$root/.tools/cadical/build/cadical"
path_label="${path//,/_}"
case_root="$root/research-results/deep-cases/w${weight}_${path_label}"

if [[ ! -x "$generator" ]]; then
  echo "missing generator: run make first" >&2
  exit 2
fi
if [[ ! -x "$solver" ]]; then
  echo "missing CaDiCaL under .tools/cadical" >&2
  exit 2
fi

mkdir -p "$case_root"
count="$(
  "$generator" \
    --anchor-weight "$weight" \
    --orbit-path "$path" \
    --list-next-orbits |
    wc -l |
    tr -d ' '
)"

run_case() {
  local next="$1"
  local stem
  stem="$(printf 'n%03d' "$next")"
  local cnf="$case_root/$stem.cnf"
  local log="$case_root/$stem.log"
  local result="$case_root/$stem.result"
  local child_path="${path},${next}"

  "$generator" \
    --centers 16 \
    --anchor-weight "$weight" \
    --orbit-path "$child_path" \
    >"$cnf"

  set +e
  "$solver" -t "$seconds" "$cnf" >"$log" 2>&1
  local rc=$?
  set -e

  local outcome="ERROR"
  if grep -q '^s SATISFIABLE' "$log"; then
    outcome="SOLVER_SAT"
  elif grep -q '^s UNSATISFIABLE' "$log"; then
    outcome="SOLVER_UNSAT"
  elif grep -q '^c UNKNOWN' "$log"; then
    outcome="UNKNOWN"
  fi
  printf '%s\t%s\t%s\n' "$next" "$outcome" "$rc" >"$result"
}

export root generator solver case_root weight path seconds
export -f run_case

seq 0 $((count - 1)) |
  xargs -P "$jobs" -n 1 bash -c 'run_case "$1"' _

cat "$case_root"/*.result | sort -n >"$case_root/results.tsv"
cat "$case_root/results.tsv"
